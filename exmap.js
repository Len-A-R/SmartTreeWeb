$(document).ready(function() {
    ...
    class Tree {
        constructor() {
            ...
            let btnSave = document.getElementById("saveToFile");
            btnSave.onclick = function(event) {
                //_self.saveToFile('./JSON/sample.json');
                download(JSON.stringify(_self), 'myMindMap.json', 'text/plain');
                //let jsonData = JSON.stringify(_self.items, ['id', 'pid', 'textItem', 'branchColor', 'text', 'selected']);

                //alert(JSON.stringify(_self));
        ...
        toJSON() {
            ...
            for (let itm of this.items.values()) {
                itemsMap.set(itm.id, {
                    ...
                    width: itm.width,
                    height: itm.height,
                    branchColor: itm.branchColor,
                    bkColor: itm.bkColor
                });
            }

            let joinsMap = new Map();
            for (let join of this.joins.values()) {
                joinsMap.set(join.id, {
                    source: join.srcItem.id,
                    dest: join.dstItem.id,
                    text: join.text,
                    startPt: join.startPoint,
                    endPt: join.endPoint,
                    startHandle: join.handleStartPoint,
                    endHandle: join.handleFinishPoint,
                    color: join.fillColor
                })
            }

            let itemObj = Object.fromEntries(itemsMap.entries());
            let joinObj = Object.fromEntries(joinsMap.entries());
            let map = new Map();
            map.set('items', itemObj);
            map.set('joins', joinObj);
        ...
        loadFromText(content) {
            let mind = JSON.parse(content);
            let items = new Map(Object.entries(mind.items));
            let joins = mind.joins;
            let _self = this;

            function loadItems(map, pid) {
                for (let key of map.keys()) {
                    let itm = map.get(key);
                    if (itm.pid === pid) {
                        let treeItem = _self.append(pid, itm.text);
                        treeItem.id = Number(key);
                        treeItem.branchColor = itm.branchColor;
                        treeItem.bkColor = itm.bkColor;
                        if (pid === 0) {
                            treeItem.x = itm.x;
                            treeItem.y = itm.y;
                        }
                        treeItem.width = itm.width;
                        treeItem.height = itm.height;
                        loadItems(map, Number(key));
                    }
                }
            }
            //загружаем корневые
            for (let key of items.keys()) {
                let itm = items.get(key);
                if (itm.pid === 0) {
                    loadItems(items, Number(key));
                }
            }
            ...
            //     if (itm) {
            //         if (key == "1") {
            //             this.root.id = Number(key);
            //             this.root.text = itm.text;
            //             this.root.pid = 0;
            //         } else {
            //             let treeItem = this.append(itm.pid, itm.text);
            //             //let treeItem = tree.find(itm.id);
            //             if (treeItem) {
            //                 treeItem.id = Number(key);
            //                 treeItem.branchColor = itm.branchColor;
            //                 treeItem.bkColor = itm.bkColor;
        ...
        set selectionRect(value) {
            ...
            this.selectionPath.segments = [
                [value.topLeft, null, null],
    ...
    class Item {
        constructor(tree, pid, text) {
            ...
            this._x = _x;
            this._y = _y;
            this._branchColor = 'blue';
            if ((this.pid !== 0) & (this.parent === tree.root)) {
                branchColorNum += 1;
                this._branchColor = branchColors[branchColorNum % branchColors.length];
            } else {
                if (this.pid === 0)
                    this._branchColor = 'white'
                else if (this.parent)
                    this._branchColor = this.parent._branchColor;
            }

            //линия итема
            this.line = new Path({ strokeColor: this._branchColor, strokeWidth: 1 });
            tree.group.addChild(this.line);
            //соединение
            this.branch = new Path({ strokeColor: this._branchColor, strokeWidth: 1 });
            tree.group.addChild(this.branch);
            //цвет
            this.branchColor = this._branchColor;
            //прямоугольник
        ...
        set branchColor(value) {
            this._branchColor = value;
            this.line.fillColor = this._branchColor;
        }
        toJSON() {
            return {
                id: this.id,
        ...
        get parent() {
            let itm = this.tree.find(this.pid);
            return itm;
        }
        get visible() {
    ...
    class Join {
        constructor(tree, srcItem, dstItem, toPoint, text) {
            ...
            tempPath.segments = [
                [srcCenter, null, srcHandle],
                [dstCenter, dstHandle, null]
            ];
    ...
    var target = document.getElementById("dropfile");
